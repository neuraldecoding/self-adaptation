...
%
% Main function with 3D Visualization
% 
...
%% Inisialization of 3D Visualization
hfig = figure;
hold on;
title(['3D Visualization of KMA for function F' num2str(FunctionID) '. Initial individuals are at the corner. Press any key to start the evolution ...'])
set(hfig, 'position', [100,100,1000,600]);
set(hfig, 'DoubleBuffer','on');
delta1  = abs(Ra(1) - Rb(1)) / 100;
delta2  = abs(Ra(2) - Rb(2)) / 100;
[xg,yg] = meshgrid(Rb(1):delta1:Ra(1),Rb(2):delta2:Ra(2));
limit1  = size(xg,1);
limit2  = size(yg,1);
...
% view(37.5,90);    % Above view
% view(37.5,75);    % High angle view
view(37.5,60);    % Low angle view
% view(37.5,0);     % Front view

h_indplot = plot3(Pop(:,1),Pop(:,2),FX,'ko',...
                       'MarkerEdgeColor','r',...
                       'MarkerFaceColor','r',...
                       'MarkerSize',5);
xlabel('x1'); ylabel('x2'); zlabel('f(x1, x2)');
...
disp('Press any key to start the evolution ...')
pause
...
while Gen < MaxGenExam2
    ...
    title(['3D Visualization of KMA in the first stage of evolution for function F', num2str(FunctionID)])
    set(h_indplot,'XData',Pop(:,1),'YData',Pop(:,2),'ZData',FX);
    xlabel('x1'); ylabel('x2'); zlabel('f(x1, x2)');
    drawnow;
    % pause(0.1)
...
if ~IsGlobal && EvalCount <= MaxNumEva
    ...
    while EvalCount < MaxNumEva
        ...
        title(['3D Visualization of KMA in the second stage of evolution for function F', num2str(FunctionID)])
        set(h_indplot,'XData',Pop(:,1),'YData',Pop(:,2),'ZData',FX);
        xlabel('x1'); ylabel('x2'); zlabel('f(x1, x2)');
        drawnow;
        % pause(0.1)
